...
queue_synchronized = queue.Queue()
...
class VideoProcessor(QObject):
    ...
    def __init__(self,cam_front,cam_side):
        ...
        self.front_cam = CameraReader(cam_front,cam_queue_front)
        self.side_cam = CameraReader(cam_side,cam_queue_side)
    ...
    def start(self,exercise):
        ...
        filename_f = dir + prefix + "_front.mp4"
        filename_s = dir + prefix + "_side.mp4"
        fourcc = cv2.VideoWriter_fourcc(*'mp4v')
        self.writer_f = cv2.VideoWriter(filename_f, fourcc, self.recorder_fps, (self.w_f, self.h_f))
        self.writer_s = cv2.VideoWriter(filename_s, fourcc, self.recorder_fps, (self.w_s, self.h_s))

        self.synchronizer = VideoSynchronizerAndWriter(cam_queue_front, cam_queue_side, queue_synchronized,self.writer_f,self.writer_s)
        self.rep_counter = RepCounterThread(queue_synchronized, exercise)
    ...
    def run(self):
        ...
        while self.running:
            ...
            if abs(ts_f - ts_s) <= 35:
                ...
                cv2.waitKey(1)
                #write to video todo: uncomment when ready
...
if __name__ == '__main__':
    vp = VideoProcessor(0,"http://192.168.65.29:8080/video")
    vp.start(0)
    ctr = 0
    while True:
        ctr += 1
